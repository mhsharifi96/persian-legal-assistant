...
- Generalized document persistence and local ingestion implemented 2026-07-18: `LegalChunkRow` no longer has fixed `book`/`bab`/`fasl`/`mabhas`/`goftar`/article/note columns; migration `0002` preserves them under `metadata.hierarchy`. `LocalFileDocumentParser` supports PDF, DOCX, modern/legacy Excel, and JSONL through lazy optional dependencies, with a generic recursive chunking fallback and `import_documents` Django command. Format-specific lineage stays in JSON metadata.
- Dadrah consultation ingestion implemented 2026-07-18: the 44,039-row nested export is handled by `DadrahJsonlParser` + `DadrahConsultationChunker`, with batched ORM upserts, question-only Qdrant indexing by default, and deterministic Neo4j `Consultation`/`Topic`/`Lawyer` relationships. `import_dadrah` makes Qdrant/Neo4j opt-in and requires explicit acknowledgement before OpenAI receives public user-generated consultation text.
