#!/usr/bin/env python3
"""Polite, resumable graph crawler for NovinLaw's public unanimity decisions.
...
Outputs a graph-ready SQLite database and JSONL exports. The crawler never
bypasses login, subscription gates, CAPTCHAs, robots.txt, or rate limits.
Subscription-gated records are retained with access_status='paywalled'.
"""
...
def detect_paywall(lines: Iterable[str]) -> bool:
    joined = "\n".join(lines)
    marker_count = sum(marker in joined for marker in PAYWALL_MARKERS)
    has_legal_content = bool(DATE_RE.search(joined) or "موضوع:" in joined or "موضوع :" in joined)
...
class Crawler:
    ...
    async def run(self) -> None:
        ...
        async with httpx.AsyncClient(
            headers=headers, follow_redirects=True, limits=limits, http2=True
        ) as client:
