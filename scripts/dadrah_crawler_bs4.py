...
The crawler uses only HTTP requests plus BeautifulSoup. For a lawyer's
...
PAGINATION_PAGE_KEYS = ("page_num", "page", "p")
PAGINATION_OFFSET_KEYS = ("offset", "start", "from", "skip")
...
def discover_load_more_targets(html: str, base_url: str) -> list[dict[str, object]]:
...
def build_pagination_strategies(
    ...
    for index, target in enumerate(discover_load_more_targets(html, advice_url), start=1):
        ...
        if lawyer_id:
            lawyer_keys = [key for key in payload if "lawyer" in key.lower()]
            if lawyer_keys:
                for key in lawyer_keys:
                    if not payload[key]:
        ...
        existing_key = next(
            (key for key in (*PAGINATION_PAGE_KEYS, *PAGINATION_OFFSET_KEYS) if key in query or key in payload),
            "",
        )
        if existing_key:
            mode = "offset" if existing_key in PAGINATION_OFFSET_KEYS else "page"
...
class DadrahCrawler:
    ...
    async def fetch_all_advices_http(self, row: sqlite3.Row) -> list[Consultation]:
        ...
        items = parse_advices(first_html, advice_url)
        ...
        for strategy in strategies:
            ...
            try:
                ...
                candidate_html = response_body_as_html(candidate_response)
                candidate_items = parse_advices(candidate_html, str(candidate_response.url))
                fresh = [item for item in candidate_items if consultation_key(item) not in initial_keys]
                if fresh:
                    accepted = strategy
                    accepted_batch = fresh
                    accepted_html = candidate_html
                    logging.info(
                        "Detected consultation pagination for %s using %s",
        ...
        if accepted is None:
            raise RuntimeError(
                "The page has 'مشاهده بیشتر', but no direct HTTP pagination/AJAX "
                "strategy returned a new consultation batch. Save the page HTML and "
                "inspect the load-more request in DevTools; no Playwright fallback is used."
            )

        all_items = items + accepted_batch
        seen = {consultation_key(item) for item in all_items}
        previous_html = accepted_html
        ...
        for batch_number in range(3, self.args.max_advice_pages + 1):
            ...
            html = response_body_as_html(response)
            page_items = parse_advices(html, str(response.url))
